...
resource "aws_lightsail_instance" "instance" {
  ...
  availability_zone = var.availability_zone == "" ? "${data.aws_region.current.name}a" : var.availability_zone
...
data "archive_file" "create_snapshots_lambda_zip" {
  type        = "zip"
  source_dir  = "${path.module}/scripts/lightsail-create-instance-snapshots"
  output_path = "${path.module}/scripts/lightsail-create-instance-snapshots.zip"
}

data "archive_file" "prune_snapshots_lambda_zip" {
  type        = "zip"
  source_dir  = "${path.module}/scripts/lightsail-prune-instance-snapshots"
  output_path = "${path.module}/scripts/lightsail-prune-instance-snapshots.zip"
}

resource "aws_lambda_function" "create_instance_snapshot_lambda" {
  filename         = "${path.module}/scripts/lightsail-create-instance-snapshots.zip"
...
resource "aws_lambda_function" "prune_instance_snapshot_lambda" {
  filename         = "${path.module}/scripts/lightsail-prune-instance-snapshots.zip"
...
resource "aws_sns_topic_subscription" "lambda_lightsail_snapshots_sns_subscription" {
  count     = var.notification_phone == "" ? 0 : 1
